...
int  Graph::chooseColor(std::set<int> validColors, std::set<int> notValidColors)
{
...
std::vector<std::vector<node>>  Graph::greedyColoring()
{
    APSP apsp(*this);
	int numberOfNodes = this->numberOfNodes();
	int diameter = Diameter::exactDiameter(*this);

	std::vector<std::vector<node>> c(numberOfNodes, std::vector<node>(diameter+1));
	std::vector<node> ordered_nodes = this->nodes();
	...
	auto it = std::find(ordered_nodes.begin(), ordered_nodes.end(), nodes[0]);
