...
std::vector<std::vector<node>> greedyColoring(Graph g)
{
  APSP apsp(g);
  int numberOfNodes = g.numberOfNodes();
  int diameter = Diameter::exactDiameter(g);

  std::vector<std::vector<node>> c(numberOfNodes+1, std::vector<node>(diameter+2));
  std::vector<node> ordered_nodes = g.nodes();
